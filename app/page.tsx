      ...
      <div className="flex justify-center">
        <div className="fades-right md:hidden w-36 h-36 rounded-full relative overflow-hidden mb-8 animate-fade-in animate-image shadow-[0_0_8px_7px_rgb(0,145,255)]">
          <Image
            src="/atef.jpg"
            alt="me"
            className="grayscale object-cover object-top scale-x-[-1]"
            fill
          />
        </div>
      </div>
      ...
      <div className="flex items-center mb-10">
        ...
        <div className="z-10 text-3xl text-transparent duration-1000 cursor-default font-display sm:text-5xl md:text-7xl lg:text-8xl bg-clip-text">
          ...
          <h2 className="fades-right text-base text-zinc-500 md:ml-1 whitespace-wrap animate-fade-in tracking-wide">
            Hi, my name is Mohamed, and I'm a{" "}
            <span className="bg-white p-1 pt-2 mx-1 text-lg text-black whitespace-nowrap">
              FULL-STACK
            </span>{" "}
            developer.
            <br />
            <div className="flex">
              <button
                className="bg-sky-400 text-white text-sm md:text-lg bg-blue rounded-xl p-3 px-8 mt-5 tracking-wider"
                onClick={(e) => {
              ...
              {open && (
                <div className="dialog-container flex fixed w-full h-full bg-gray-800 bg-opacity-50 items-center justify-center top-0 left-0">
                  <div className="dialog bg-white p-4 rounded-2xl w-full max-w-xl relative m-3">
                    <button
                      className="close absolute top-0 right-0 text-gray-500 m-4 border px-2 rounded"
                      onClick={() => {
                        setAnswer(undefined);
                        setOpen(false);
                        setPrompt("");
                    ...
                    <h2 className="text-center text-black mb-3 text-3xl">
                      ASK GPT
                    </h2>
                    <h3 className=" text-gray-900 font-sans">
                      A chance to ask GPT while you are here...
                    </h3>

                    {answer ? (
                      <div className="answer bg-black text-white font-thin font-mono p-3 rounded-lg mt-3">
                        <div className=" text-green-600 mb-2">Q: {prompt}</div>{" "}
                        A: {answer}
                      </div>
                    ) : (
                      <textarea
                        onChange={(e) => setPrompt(e.target.value)}
                        className="border border-1 border-gray-300 selection:border-grey-400 w-full p-2 mt-2 rounded"
                        placeholder="How are you doing GPT?"
                        rows={4}
                        aria-expanded={false}
                        autoFocus
                        autoCorrect="false"
                    ...
                    {answer && (
                      <div className="flex justify-between">
                        <button
                          ...
                          className="p-2 rounded px-4 text-white mt-3"
                          style={{
                            backgroundColor: asking ? "gray" : "black",
                          }}
                        >
